...
void CmsShowMainBase::setupActions() {
  m_guiManager->writeToPresentConfigurationFile_.connect(
      sigc::mem_fun(*this, &CmsShowMainBase::writeToCurrentConfigFile));
  ...
  if (m_guiManager->getAction(cmsshow::sPreviousEvent) != nullptr)
    m_guiManager->getAction(cmsshow::sPreviousEvent)
        ->activated.connect(sigc::mem_fun(*this, &CmsShowMainBase::doPreviousEvent));
  if (m_guiManager->getAction(cmsshow::sGotoFirstEvent) != nullptr)
    m_guiManager->getAction(cmsshow::sGotoFirstEvent)
        ->activated.connect(sigc::mem_fun(*this, &CmsShowMainBase::doFirstEvent));
  if (m_guiManager->getAction(cmsshow::sGotoLastEvent) != nullptr)
    m_guiManager->getAction(cmsshow::sGotoLastEvent)
        ->activated.connect(sigc::mem_fun(*this, &CmsShowMainBase::doLastEvent));
...
void CmsShowMainBase::setupViewManagers() {
  ...
  configurationManager()->add(std::string("L1TriggerTables"),
                              triggerTableViewManager.get());  // AMT: added for backward compatibilty
  triggerTableViewManager->setContext(m_contextPtr);
  viewManager()->add(triggerTableViewManager);

  auto geoTableViewManager =
      std::make_shared<FWGeometryTableViewManager>(guiManager(), m_simGeometryFilename, m_TGeoName);
...
void CmsShowMainBase::eventChangedSlot() { eventChangedImp(); }

void CmsShowMainBase::eventChangedImp() { guiManager()->eventChangedCallback(); }
...
void CmsShowMainBase::goToRunEvent(edm::RunNumber_t run, edm::LuminosityBlockNumber_t lumi, edm::EventNumber_t event) {
...
void CmsShowMainBase::draw() {
  ...
  sw.Stop();
  fwLog(fwlog::kDebug) << "CmsShowMainBase::draw CpuTime " << sw.CpuTime() << " RealTime " << sw.RealTime()
                       << std::endl;
...
void CmsShowMainBase::writeToCurrentConfigFile() { m_configurationManager->writeToFile(m_configFileName); }

void CmsShowMainBase::partialWriteToConfigFile(const std::string &name) {
...
void CmsShowMainBase::reloadConfiguration(const std::string &config) {
  ...
  std::string msg = "Reloading configuration " + config + "...";
...
void CmsShowMainBase::partialLoadConfiguration(const std::string &name) {
...
void CmsShowMainBase::setupConfiguration() {
  ...
  try {
    ...
    fwLog(fwlog::kError) << "Unable to load configuration file '" << m_configFileName << "': " << e.error()
                         << std::endl;
    exit(1);
  } catch (std::runtime_error &e) {
    fwLog(fwlog::kError) << "Unable to load configuration file '" << m_configFileName
                         << "' which was specified on command line. Quitting." << std::endl;
    exit(1);
  }
}

void CmsShowMainBase::setPlayDelay(Float_t val) { m_playDelay = val; }
...
void CmsShowMainBase::setPlayLoopImp() { m_loop = true; }

void CmsShowMainBase::unsetPlayLoopImp() { m_loop = false; }

void CmsShowMainBase::registerPhysicsObject(const FWPhysicsObjectDesc &iItem) { m_eiManager->add(iItem); }
...
void CmsShowMainBase::loadGeometry() {  // prepare geometry service
...
void CmsShowMainBase::sendVersionInfo() {
  ...
  int flags = 0;
  sendto(sd, msg, strlen(msg), flags, (struct sockaddr *)&remoteServAddr, sizeof(remoteServAddr));
}
