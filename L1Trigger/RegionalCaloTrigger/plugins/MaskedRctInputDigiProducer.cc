	   ...
	   if (sign < 0)
	     {
	       ...
	       fineGrain = ( ecalMask.at(0).at(iphi-1).at(absIeta-1) * ecalColl[i].sample(nSample).fineGrain() ) != 0;
	     }
	   else if (sign > 0)
	     {
	       ...
	       fineGrain = ( ecalMask.at(1).at(iphi-1).at(absIeta-1) * ecalColl[i].sample(nSample).fineGrain() ) != 0;
	     }
       ...
       for (int nSample = 0; nSample < nHcalSamples; nSample++)
	 {
	   ...
	   if (absIeta < 29)
	       ...
	       if (sign < 0)
		 {
		   ...
		   fineGrain = ( hcalMask.at(0).at(iphi-1).at(absIeta-1) * hcalColl[i].sample(nSample).fineGrain() ) != 0;
		 }
	       else if (sign > 0)
		 {
		   energy = hcalMask.at(1).at(iphi-1).at(absIeta-1) * hcalColl[i].sample(nSample).compressedEt();
		   fineGrain = ( hcalMask.at(1).at(iphi-1).at(absIeta-1) * hcalColl[i].sample(nSample).fineGrain() ) != 0;
		 }
	     }
	       ...
	       if (sign < 0)
		 {
		   ...
		   fineGrain = ( hfMask.at(0).at(hf_phi_index).at(absIeta-29) * hcalColl[i].sample(nSample).fineGrain() ) != 0;
		 }
	       else if (sign > 0)
		 {
		   //std::cout << "ieta is " << ieta << ", absIeta is " << absIeta << ", iphi is " << iphi << std::endl;
		   ...
		   fineGrain = ( hfMask.at(1).at(hf_phi_index).at(absIeta-29) * hcalColl[i].sample(nSample).fineGrain() ) != 0;
		 }
